...
## 📝 EDITORIAL NOTE — STRUCTURE DECISION

> **Do not edit the narration before the interviews are recorded.**

Current narration: ~1 900 words → ~19 min at HeyGen pacing (100 wpm).
Target episode: ~10 min total.
Budget once interviews are in: ~700–800 words of narration max.

**The interviews ARE the episode.**
Bob, Mike and Andy's words carry the real value —
the narration exists only to set the scene and connect the dots between their answers.

Editing strategy (post-interviews):
- Keep the SDI/Star Wars hook — it's too good to cut.
- Keep the Persuaders frame — it's the episode identity.
- Everything else in the narration is a candidate for cuts or compression.
- Let the guests tell the facts. The narration tells the feeling.

**Next hard step: get the recordings. Everything else can wait.**
